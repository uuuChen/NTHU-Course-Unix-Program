...
void make_sure_setting_done(void){
    make_sure_handle_exist();
...
static void afterMain(void){
    if (dlclose(handle) != 0){
	fprintf(output_file, "Close Handle Error...\n");
	exit(EXIT_FAILURE);
    }
}

int chdir(const char *path){
    fprintf(output_file, "[sandbox]: in %s\n", __func__);
